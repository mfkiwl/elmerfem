...
!------------------------------------------------------------------------------
     ...
     CHARACTER(LEN=MAX_NAME_LEN) :: ModelName, ExecCommand, ExtrudedMeshName, str
     ...
     LOGICAL :: DoControl, ExtControl, ProcControl, GotParams
     INTEGER :: nr,ni
     ...
     IF ( NoArgs > 0 ) THEN
       ...
       IF( ModelName(1:1) /= '-') THEN 
         GotModelName = .TRUE.
         IF (NoArgs > 1) CALL GET_COMMAND_ARGUMENT(2, str)
       END IF
     END IF
...
!------------------------------------------------------------------------------
       str = ListGetString( CurrentModel % Simulation, 'Simulation Type', GotIt )
...
!------------------------------------------------------------------------------
      ...
      DO i=1,CurrentModel % NumberOfSolvers
        ...
        str = ListGetString( CurrentModel % Solvers(i) % Values,'Equation', Found )
             ...
             IF ( Found ) THEN
                DO k=1,SIZE(ActiveSolvers)
                   IF ( ActiveSolvers(k) == i ) THEN
                      CALL ListAddLogical( CurrentModel % Equations(j) % Values, str(1:nlen), .TRUE. )
                      EXIT
                   END IF
                END DO
             END IF
     ...
     DO i=1,CurrentModel % NumberOfSolvers
        str = ListGetString( CurrentModel % Solvers(i) % Values,'Equation', Found )
        CALL Info('AddSolvers','Setting up solver '//TRIM(I2S(i))//': '//TRIM(str),Level=10)
        ...
        IF ( Solver % PROCEDURE == 0 .OR. InitSolver ) THEN
          ...
          END IF
          CurrentModel % Solver => Solver
          CALL AddEquationBasics( Solver, str, Transient )
